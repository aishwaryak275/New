...
public class ServiceRequestController {
    ...
    // Allow either the SERVICE_REQUEST authority (agents) or RAISE_SERVICE_REQUEST (subscribers)
    ...
    // POST /teleConnect/fault/requests/{requestId}/approve-connection
    @PostMapping("/requests/{requestId}/approve-connection")
    @PreAuthorize("hasAnyAuthority('SERVICE_REQUEST','CREATE_USER','VIEW_SUBSCRIBER','CREATE_SUB')")
    public ResponseEntity<MessageResponse> approveConnection(
            @PathVariable Integer requestId,
            HttpServletRequest httpReq) {
        log.info("Approving connection request requestId={}", requestId);
        String authHeader = httpReq.getHeader("Authorization");
        MessageResponse result = requestService.approveConnection(requestId, authHeader);
        auditClient.record(AuditAction.UPDATE_SERVICE_REQUEST, AuditModule.FAULT, httpReq);
        log.info("Service request connection approved requestId={}", requestId);
        return ResponseEntity.ok(result);
    }
}
