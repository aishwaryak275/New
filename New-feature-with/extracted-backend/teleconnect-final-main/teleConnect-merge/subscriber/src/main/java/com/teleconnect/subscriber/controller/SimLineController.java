...
public class SimLineController {
    ...
    @PreAuthorize("hasAnyAuthority('CREATE_USER','VIEW_SUBSCRIBER','CREATE_SUB')")
    ...
    @GetMapping("/sim-lines/lookup-by-id")
    @PreAuthorize("hasAnyAuthority('VIEW_ALL_USERS','VIEW_SUBSCRIBER','GET_SUB')")
    public ResponseEntity<SimLineResponseDTO> lookupByLineId(
            @RequestParam Integer lineId) {
        log.info("Looking up SIM line by lineId={}", lineId);
        SimLineResponseDTO line = simLineService.lookupByLineId(lineId);
        return ResponseEntity.ok(line);
    }
