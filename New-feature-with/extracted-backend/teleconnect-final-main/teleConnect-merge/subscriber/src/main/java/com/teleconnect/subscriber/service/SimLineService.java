...
public class SimLineService {
    ...
    public MessageDTO createSimLine(Integer accountId,
        ...
        return new MessageDTO("SIM line activated successfully", accountId, simLine.getLineId());
    }
    ...
    public SimLineResponseDTO lookupByLineId(Integer lineId) {
        log.debug("Lookup SIM line by lineId={}", lineId);
        SimLine simLine = simLineRepo.findById(lineId)
            .orElseThrow(() -> new RuntimeException(
                "No SIM line found for ID: " + lineId));
        return toDTO(simLine);
    }
